...
{  
    if(argc < 5)
    {
        cerr << endl << "Usage: ./stereo_ocams path_to_vocabulary path_to_settings path_to_sequence_folder_1 path_to_times_file_1 (path_to_image_folder_2 path_to_times_file_2 ... path_to_image_folder_N path_to_times_file_N) (trajectory_file_name)" << endl;
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        int num_rect = 0;
        int proccIm = 0;
        for(int ni=700; ni<nImages[seq]; ni++, proccIm++)
        {
            cout << vstrImageLeft[seq][ni] << endl;
            // Read left and right images from file
            cv::Mat imLeftTmp, imRightTmp;
            imLeftTmp = cv::imread(vstrImageLeft[seq][ni], cv::IMREAD_UNCHANGED);
            cv::cvtColor(imLeftTmp, imLeft, cv::COLOR_BGR2GRAY);
            imRightTmp = cv::imread(vstrImageRight[seq][ni], cv::IMREAD_UNCHANGED);
            cv::cvtColor(imRightTmp, imRight, cv::COLOR_BGR2GRAY);
