...
int main(int argc, char **argv)
{
    if(argc < 5)
    {
        cerr << endl << "Usage: ./stereo_inertial_ocams path_to_vocabulary path_to_settings path_to_sequence_folder_1 path_to_times_file_1 (path_to_image_folder_2 path_to_times_file_2 ... path_to_image_folder_N path_to_times_file_N) " << endl;
        return 1;
    }
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=700; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            imLeft = cv::imread(vstrImageLeft[seq][ni], cv::IMREAD_COLOR);
            cv::cvtColor(imLeft, imLeft, cv::COLOR_BGR2GRAY);
            imRight = cv::imread(vstrImageRight[seq][ni], cv::IMREAD_COLOR);
